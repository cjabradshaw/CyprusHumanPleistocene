...
#####################################################
## stagger arrival times of founding population    ##
...
for (f in 1:length(propSamp.vec)) {
  ...
  # size of first arriving group
  prop.alpha <- estBetaParams(propSamp.vec[f], propSamp.varProp*propSamp.vec[f])$alpha
  prop.beta <- estBetaParams(propSamp.vec[f], propSamp.varProp*propSamp.vec[f])$beta
  found.st <- round(pop.found * rbeta(1, prop.alpha, prop.beta), 0)  
  found.st
  init.vec <- round(stable.stage.dist(popmat) * found.st/2, 0) # stable stage distribution x founding population size (female only)
  ...
  for (e in 1:iter) { # e iterations loop
    ...
    events.it <- round(pop.found/round(pop.found*propSamp.vec[f], 0), 0) # number of immigration events
    events.times <- round(runif(events.it-1, min=round(rnorm(1, min.int, 1), 0), max=round(rnorm(1, max.int, 0.1*max.int),0)), 0) # timing of immigration events
    events.times.cum <- cumsum(events.times)
    
    ## size of immigration events
    prop.alpha <- estBetaParams(propSamp.vec[f], propSamp.varProp*propSamp.vec[f])$alpha
    prop.beta <- estBetaParams(propSamp.vec[f], propSamp.varProp*propSamp.vec[f])$beta
    props.it <- rbeta(events.it, prop.alpha, prop.beta)  
    arrives.it <- round(props.it * pop.found, 0)
  ...
  if (f > 1) {
    plot(propSamp.vec, pr.ext.vec, type="l", lty=2, col="red", xlab="mean prop founding arriving per interval", ylab="Pr(ext)")
  }
...
plot(immig.int.mn, pr.ext.vec, type="l", lty=1, col="black", xlab="founding period", ylab="Pr(Qext)",
     xlim=c(min(immig.int.lo), max=max(immig.int.up)))
lines(immig.int.lo, pr.ext.vec, lty=2, col="red")
lines(immig.int.up, pr.ext.vec, lty=2, col="red")

plot(propSamp.vec, immig.int.mn, type="l", lty=1, col="black", xlab="mean prop founding arriving per interval",
     ylab="founding period (years)")
lines(propSamp.vec, immig.int.lo, lty=2, col="red")
