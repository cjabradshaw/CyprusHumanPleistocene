...
for (f in 1:length(propSamp.vec)) {
  ...
  init.vec <- stable.stage.dist(popmat) * found.st/2 # stable stage distribution x founding population size (female only)
  ...
  for (e in 1:iter) { # e iterations loop
    ...
    events.times <- round(runif(events.it-1, min=rnorm(min.int, 0.05*min.int), max=rnorm(max.int, 0.05*max.int)),0) # timing of immigration events
